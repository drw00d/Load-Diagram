...
def spot_letter(spot: int) -> str:
    # LoadXpert-like A/B stagger tags (except doorway uses no-stagger labels)
    # We label A for "up" and B for "down" outside doorway.
    return "A" if (spot % 2 == 1) else "B"


def summarize_spot(matrix: List[List[Optional[str]]], spot: int) -> List[Tuple[str, int]]:
    """Return list of (pid, count) per spot, bottom->top (tier order)."""
    col = matrix[spot - 1]
    seq = [pid for pid in col if pid and pid != BLOCK]
    if not seq:
        return []
...
# =============================
# TOP VIEW (UPDATED)
# - shows stagger outside doorway
# - TURN shows exactly 2 horizontal footprints
# =============================
def render_top_svg(
    *,
    car_id: str,
    ...
    # spot footprint geometry
    base_h = lane_h * 0.70
    base_y = y0 + (lane_h - base_h) / 2

    # stagger settings
    stagger_px = 18  # how much "down" to shift B spots
    def spot_y(spot: int) -> float:
        if DOOR_START_SPOT <= spot <= DOOR_END_SPOT:
            return base_y  # no stagger in doorway 6-9
        # alternate A/B
        return base_y if (spot_letter(spot) == "A") else (base_y + stagger_px)

    # spot rectangles (top view: 1 footprint per spot)
    spot_rect: Dict[int, Tuple[float, float, float, float]] = {}
    for s in range(1, FLOOR_SPOTS + 1):
        x = x0 + (s - 1) * cell_w + cell_w * 0.10
        bw = cell_w * 0.80
        y = spot_y(s)
        bh = base_h - (stagger_px if (s < DOOR_START_SPOT or s > DOOR_END_SPOT) else 0)
        # keep within bounds
        bh = min(bh, y0 + lane_h - y - 6)
        spot_rect[s] = (x, y, bw, bh)

    # TURN spot summary (how many tiers are turned)
    turn_summary = summarize_spot(matrix, turn_spot)
    turn_pid = turn_summary[0][0] if turn_summary else None
    turn_count = sum(n for _, n in turn_summary) if turn_summary else 0
    ...
    svg.append(f'<text x="{margin+10}" y="{margin+26}" font-size="18" font-weight="700">Car: {car_id} — Top View</text>')
    svg.append(f'<text x="{margin+10}" y="{margin+50}" font-size="13">{note}</text>')
    ...
    svg.append(f'<text x="{door_left+6}" y="{y0-6}" font-size="12" fill="#c00000">Doorway (Spots {DOOR_START_SPOT}-{DOOR_END_SPOT})</text>')
    ...
    # draw each spot footprint with labels
    for s in range(1, FLOOR_SPOTS + 1):
        x, y, bw, bh = spot_rect[s]

        # blocked spot still shows as blank footprint
        if s == blocked:
            svg.append(f'<rect x="{x}" y="{y}" width="{bw}" height="{bh}" fill="#ffffff" stroke="#333" stroke-width="1"/>')
            svg.append(f'<text x="{x+6}" y="{y+16}" font-size="12" fill="#333">{s}</text>')
            svg.append(f'<text x="{x+6}" y="{y+34}" font-size="11" fill="#555">BLOCKED</text>')
            continue

        # spot summary label
        summ = summarize_spot(matrix, s)
        rep = summ[0][0] if summ else None
        ...
        # "spot number" plus A/B tag outside doorway
        if DOOR_START_SPOT <= s <= DOOR_END_SPOT:
            spot_tag = f"{s}"
        else:
            spot_tag = f"{s}{spot_letter(s)}"
        svg.append(f'<text x="{x+6}" y="{y+16}" font-size="12" fill="#333">{spot_tag}</text>')

        # Doorframe no-ME outline
        ...
        # write up to 2 lines of "pid xN" like LoadXpert top
        if summ:
            line_y = y + 36
            for pid, cnt in summ[:2]:
                hp = " HP" if (pid in products and products[pid].is_half_pack) else ""
                svg.append(f'<text x="{x + bw/2}" y="{line_y}" font-size="12" text-anchor="middle">{pid}{hp} x{cnt}</text>')
                line_y += 18
            if len(summ) > 2:
                svg.append(f'<text x="{x + bw/2}" y="{line_y}" font-size="12" text-anchor="middle">+{len(summ)-2} more</text>')

    # TURN (Top view): show EXACTLY 2 horizontal footprints (not stacked blocks)
    # We draw two "turned units" inside the 2-spot span: one in the left half, one in the right half.
    if turn_pid and turn_count > 0 and tiers > 0:
        # span area
        xL, yL, bwL, bhL = spot_rect[turn_spot]
        xR, yR, bwR, bhR = spot_rect[blocked]
        span_x = xL
        span_y = min(yL, yR)
        span_w = (xR + bwR) - xL
        span_h = max(bhL, bhR)

        # make two horizontal footprints (wide-ish, shorter height)
        pad = 8
        inner_x = span_x + pad
        inner_y = span_y + pad
        inner_w = span_w - 2 * pad
        inner_h = span_h - 2 * pad

        # two “turned” rectangles: stacked vertically so you visually see "2 units"
        h_unit = inner_h * 0.42
        gap = inner_h * 0.08
        y1 = inner_y + (inner_h - (2*h_unit + gap)) / 2
        y2 = y1 + h_unit + gap

        fill = color_for_pid(turn_pid)
        hp = " HP" if (turn_pid in products and products[turn_pid].is_half_pack) else ""

        for yi in [y1, y2]:
            svg.append(f'<rect x="{inner_x}" y="{yi}" width="{inner_w}" height="{h_unit}" fill="{fill}" opacity="0.92" stroke="#111" stroke-width="2"/>')
            svg.append(f'<text x="{inner_x + 12}" y="{yi + h_unit/2 + 5}" font-size="12" font-weight="700" fill="#111">TURN</text>')
            svg.append(f'<text x="{inner_x + inner_w/2}" y="{yi + h_unit/2 + 5}" font-size="12" text-anchor="middle">{turn_pid}{hp}</text>')

        svg.append(f'<rect x="{inner_x-2}" y="{y1-2}" width="{inner_w+4}" height="{(y2+h_unit)-(y1)+4}" fill="none" stroke="#111" stroke-width="2"/>')
...
def render_side_svg(
    *,
    ...
    # TURN tiers render merged in Side view (looks like LoadXpert)
...
if view_mode == "Top only":
    components_svg(top_svg, height=400)
